...
def run_assembler(resource_path,reads_path, sample_d, output_dir,runD):
    ...
    if not(os.path.exists(output_dir+"/"+runD)):
            ...
            for sample in sample_d:
                ...
                #subprocess.run("/epi/home/ashita.jawali@kdhe.state.ks.us/miniconda3/bin/python3.9 /Users/adrian/Desktop/SPAdes-3.15.5-Darwin/bin/spades.py --isolate -o"+output_dir+"/"+sample+" --pe1-1 "+reads_path+"/fastp/"+sample+"/"+sample_d[sample][0]+" --pe1-2 "+reads_path+"/fastp/"+sample+"/"+sample_d[sample][1], shell=True)
...
def check_assembly_qual(resource_path,path_to_fasta,qual_outputdir,samples,runD):
    ...
    for sample in samples:
        ...
        #print(". $CONDA_PREFIX/epi/home/ashita.jawali@kdhe.state.ks.us/mambaforge/etc/profile.d/conda.sh && conda activate BUSCO_env && busco -i "+path_to_fasta+"/"+runD+"/"+sample+"/scaffolds.fasta -l "+resource_path+"/resources/busco/bacteria_odb10/ -o "+sample+"_busco --out_path "+qual_outputdir+" -m genome --offline")
        subprocess.run("bash -c 'source /epi/home/ashita.jawali@kdhe.state.ks.us/miniconda3/bin/activate BUSCO && busco -i "+path_to_fasta+"/"+runD+"/"+sample+"/scaffolds.fasta -l "+resource_path+"/resources/busco/bacteria_odb10/ -o "+sample+"_busco --out_path "+qual_outputdir+"/ -m genome --offline'", shell=True)
